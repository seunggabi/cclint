...
export type { LintResult, FixSuggestion, CcLintConfig } from './types.js';
...
export function lint(command: string, config: CcLintConfig = DEFAULT_CONFIG): LintOutput {
...
export function lintWithFix(command: string, config: CcLintConfig = DEFAULT_CONFIG): LintOutput {
  const ast = buildAST(command);
  const messages = runRules(ast, config);
  const result = buildLintResult(command, messages);
